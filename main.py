...
async def on_message(message):
    ...
    if isinstance(message.channel, discord.DMChannel):
        ...
        sent_message = await message.channel.send(text("bot.send_to_admin_ask"), view=view)
...
class DMConfirmView(discord.ui.View):
    ...
    @discord.ui.button(label=text("bot.yes"), style=discord.ButtonStyle.green)
    async def yes(self, interaction: discord.Interaction, button: discord.ui.Button):
        admin = bot.get_user(int(os.getenv("ADMIN_DCID")))
        if admin:
            await admin.send(text("bot.message_from", self.original_message.author, self.original_message.content))
            await interaction.response.send_message(text("bot.sent_to_admin"), ephemeral=True)
        else:
            await interaction.response.send_message(text("bot.sent_to_admin_err"), ephemeral=True)
        await self.message.delete()

    @discord.ui.button(label=text("bot.no"), style=discord.ButtonStyle.red)
...
if not os.getenv("PASSWORD"):
    ...
    class EnterPswView(discord.ui.Modal, title=text("bot.enterpsw.title")):
        ...
        async def on_submit(self, interaction: discord.Interaction):
            if os.getenv("PASSWORD") != self.password_box.value:
    ...
    @bot.hybrid_command(name="load", description=text("load.description"))
    async def load(ctx: commands.Context, extension: str):
        await ctx.interaction.response.send_modal(EnterPswView(do="load", extension=extension))

    @bot.hybrid_command(name="unload", description=text("unload.description"))
    async def unload(ctx: commands.Context, extension: str):
        await ctx.interaction.response.send_modal(EnterPswView(do="unload", extension=extension))

    @bot.hybrid_command(name="reload", description=text("reload.description"))
    async def reload(ctx: commands.Context, extension: str):
        await ctx.interaction.response.send_modal(EnterPswView(do="reload", extension=extension))

    @bot.hybrid_command(name="senddm", description=text("senddm.description"))
    async def senddm(ctx: commands.Context, message: str, user_id: int, password: str):
        if password != os.getenv("PASSWORD"):
            await ctx.interaction.response.send_message(text("bot.incorrect_psw"), ephemeral=True)
            return
        try:
            user = await bot.fetch_user(user_id)
            await user.send(message)
            await ctx.interaction.response.send_message(text("senddm.sent", user_id), ephemeral=True)
        except Exception as e:
            await ctx.interaction.response.send_message(text("senddm.failed", e), ephemeral=True)
